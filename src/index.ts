...
async function analyzeSignature(connection: Connection, signature: string) {
  ...
  if (transactionDetails?.meta?.logMessages) {
    ...
    const isRaydiumLog = logs.some(
      (log) => log.includes(RAYDIUM_LIQUIDITYPOOL_V4.toString()) //675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8
    );
    const isTransferLog = logs.some((log) => log.includes('Program log: Instruction: Transfer'));
